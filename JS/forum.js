...
// Seleciona o container onde os posts serão exibidos
            ...
            posts.forEach(async post => {
                ...
                postElement.innerHTML = `
                    <div class="card-body">
                        ...
                        <ul class="list-group" id="comments-${post.id}"></ul>
                    </div>
                `;

                postsContainer.appendChild(postElement);

                // Busca e renderiza os comentários
                const commentsList = document.getElementById(`comments-${post.id}`);
                const comments = await fetchComments(post.id);

                if (comments.length === 0) {
                    commentsList.innerHTML = `<li class="list-group-item text-muted">Sem comentários ainda.</li>`;
                } else {
...
// Chamada inicial para buscar posts
fetchPosts();
